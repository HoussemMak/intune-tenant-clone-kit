...
<#
.SYNOPSIS
    Nettoie les objets crees par une tentative d'import Intune echouee.

.DESCRIPTION
    Lit RAPPORT-IMPORT*.txt, extrait les objets marques CREATED, se connecte au tenant cible,
    recherche les objets par nom dans la collection Graph attendue, et supprime uniquement les
    candidats crees dans la fenetre temporelle d'import fournie. Le mode par defaut est l'apercu.
    Ajouter -Execute pour supprimer.

.NOTES
    La fenetre -ImportStartUtc / -ImportEndUtc empeche de supprimer les objets preexistants : seuls
    les objets dont le createdDateTime s'y trouve sont supprimes. Par defaut, les dernieres 24 heures
    (nettoyage juste apres une execution echouee). Fournir une fenetre explicite et etroite pour
    nettoyer un import plus ancien.
#>
[CmdletBinding()]
param(
...
function Save-Results(){
 if($Results.Count -gt 0){ $Results | Export-Csv -Path $LogPath -NoTypeInformation -Encoding UTF8; Write-Host "Journal: $LogPath" -ForegroundColor Cyan }
}
...
try{
 if(-not (Test-Path -LiteralPath $ReportPath)){ throw "ReportPath introuvable: $ReportPath" }
 if(-not $Execute){ Write-Host 'Mode PREVIEW: aucune suppression. Ajouter -Execute pour supprimer.' -ForegroundColor Yellow }
 Import-Module Microsoft.Graph.Authentication -ErrorAction Stop
 Connect-GraphForIntuneAutomation -TenantId $TargetTenantId -Scopes $Scopes
 $ctx=Get-MgContext
 Write-Host "Tenant cible: $($ctx.TenantId)" -ForegroundColor Cyan
 $entries=@(Get-CreatedEntriesFromReport)
 Write-Host "Objets CREATED dans le rapport: $($entries.Count)" -ForegroundColor Cyan
 if($Execute -and (-not $Force)){
  $r=Read-Host "Supprimer les objets crees entre $($ImportStartUtc.ToString('s'))Z et $($ImportEndUtc.ToString('s'))Z ? [O/N]"
  if($r -notmatch '^[OoYy]'){ throw 'Annule par utilisateur.' }
 }
 foreach($g in ($entries | Group-Object Family)){
  if(-not $Catalog.ContainsKey($g.Name)){
   foreach($e in $g.Group){ Add-Result $e.Family $e.Name 'SKIPPED' 'NO_CATALOG' $null $null 'Famille non supportee par cleanup' $null }
   continue
  }
  if($DisableScopeTags -and $g.Name -eq '08_ScopeTags'){
   foreach($e in $g.Group){ Add-Result $e.Family $e.Name 'SKIPPED' 'RBAC_SCOPE_TAGS_DISABLED' $null $null 'Lecture/suppression ScopeTags ignoree car DeviceManagementRBAC non disponible ou desactive.' $null }
   Write-Host "Lecture cible: $($g.Name) ignoree (RBAC ScopeTags desactive)." -ForegroundColor Yellow
   continue
  }
  $cat=$Catalog[$g.Name]
  Write-Host "Lecture cible: $($g.Name)" -ForegroundColor Cyan
  try { $items=@(Get-AllGraphItems -Uri $cat.Path) }
  catch {
   foreach($e in $g.Group){ Add-Result $e.Family $e.Name 'SKIPPED' 'LOOKUP_FORBIDDEN_OR_FAILED' $null $null 'Lecture cible impossible; nettoyage ignore pour cette famille.' $_.Exception.Message }
   Write-Host "  [SKIPPED] Lecture cible impossible pour $($g.Name): $($_.Exception.Message)" -ForegroundColor Yellow
   continue
  }
  foreach($e in $g.Group){
   $matches=@($items | Where-Object { (Get-PropertyValue $_ $cat.NameProp) -eq $e.Name })
   if($matches.Count -eq 0){ Add-Result $e.Family $e.Name 'NOT_FOUND' 'LOOKUP' $null $null 'Aucun objet cible du meme nom' $null; continue }
   foreach($m in $matches){
    ...
    if(-not $inWindow){ Add-Result $e.Family $e.Name 'SKIPPED' 'DATE_GUARD' $id $created 'createdDateTime hors fenetre import ou absent' $null; continue }
    if(-not $Execute){ Add-Result $e.Family $e.Name 'PREVIEW_DELETE' 'DELETE' $id $created 'Suppression candidate' $null; Write-Host "  [PREVIEW DELETE] $($e.Name)" -ForegroundColor Gray; continue }
